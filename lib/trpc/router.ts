...
export const appRouter = t.router({
  ...
  mobile: t.router({
    ...
    leaveEvent: protectedProcedure
      ...
      .mutation(({ input }) => leaveEvent(input.eventId)),
    joinEventFromInvite: protectedProcedure
      .input(
        z.object({
      ...
      .mutation(async ({ input }) => {
        ...
        const result = await joinEvent(event.id, input.inviteCode);
        return { ...result, slug: event.slug };
      }),
    ...
    finalizePhoto: protectedProcedure
      .input(
        z.object({
      ...
      .mutation(({ input }) =>
        finalizeUpload(input.mediaId, input.eventId, {
          ...
          thumbnailError: "Native thumbnail generation skipped",
        }),
      ),
    mediaLikes: protectedProcedure
      .input(z.object({ mediaId: z.string().uuid() }))
      .query(({ input }) => getMediaLikes(input.mediaId)),
    toggleMediaLike: protectedProcedure
      .input(z.object({ mediaId: z.string().uuid() }))
      .mutation(({ input }) => toggleMediaLike(input.mediaId)),
    mediaComments: protectedProcedure
      .input(z.object({ mediaId: z.string().uuid() }))
      .query(({ input }) => getMediaComments(input.mediaId)),
    createComment: protectedProcedure
      ...
      .mutation(({ input }) =>
        createComment(input.mediaId, input.content, input.parentCommentId),
      ),
    toggleCommentLike: protectedProcedure
      .input(z.object({ commentId: z.string().uuid() }))
      .mutation(({ input }) => toggleCommentLike(input.commentId)),
    deleteComment: protectedProcedure
      .input(z.object({ commentId: z.string().uuid() }))
      .mutation(({ input }) => deleteComment(input.commentId)),
    createShareLink: protectedProcedure
      .input(
        z.object({
          mediaId: z.string().uuid(),
          type: z.enum(["view", "raw"]).default("view"),
        }),
      )
      .mutation(({ input }) => createShareLink(input.mediaId, input.type)),
    reportMedia: protectedProcedure
      ...
      .mutation(({ input }) => createReport(input.mediaId, input.reason)),
    deleteMedia: protectedProcedure
      .input(z.object({ mediaId: z.string().uuid() }))
      .mutation(({ input }) => deleteMedia(input.mediaId)),
    updateCaption: protectedProcedure
      .input(
        z.object({ mediaId: z.string().uuid(), caption: z.string().max(500) }),
      )
      .mutation(({ input }) =>
        updateMediaCaption(input.mediaId, input.caption),
      ),
    mediaTags: protectedProcedure
      .input(z.object({ mediaId: z.string().uuid() }))
      .query(({ input }) => getMediaTags(input.mediaId)),
    addTag: protectedProcedure
      ...
      .mutation(({ input }) => addTag(input.mediaId, input.tagName)),
    removeTag: protectedProcedure
      .input(z.object({ mediaId: z.string().uuid(), tagId: z.string().uuid() }))
      .mutation(({ input }) => removeTag(input.mediaId, input.tagId)),
    searchTags: protectedProcedure
      .input(z.object({ query: z.string().min(0).max(80) }))
      .query(({ input }) => searchByTag(input.query)),
    mediaMentions: protectedProcedure
      .input(z.object({ mediaId: z.string().uuid() }))
      .query(({ input }) => getMediaMentions(input.mediaId)),
    addMention: protectedProcedure
      .input(
        z.object({ mediaId: z.string().uuid(), userId: z.string().uuid() }),
      )
      .mutation(({ input }) => addMention(input.mediaId, input.userId)),
    removeMention: protectedProcedure
      .input(
        z.object({ mediaId: z.string().uuid(), userId: z.string().uuid() }),
      )
      .mutation(({ input }) => removeMention(input.mediaId, input.userId)),
    ...
    series: t.procedure.query(async ({ ctx }) => {
      ...
      const visibleSeries = userContext
        ? allSeries
        : allSeries.filter((series) => series.visibility === "public");
      const eventIds = visibleSeries.flatMap((series) =>
        series.events.map((event) => event.id),
      );
      const mediaCounts = eventIds.length
        ? await db
            .select({ eventId: media.eventId, count: count() })
            .from(media)
            .where(inArray(media.eventId, eventIds))
      ...
      return visibleSeries.map((series) => ({
        ...
        bannerUrl: series.bannerS3Key
          ? getAssetProxyUrl("series-banner", series.id)
          : null,
        eventCount: series.events.length,
        totalPhotos: series.events.reduce(
          (total, event) => total + (mediaCountByEventId.get(event.id) ?? 0),
          0,
        ),
      }));
    }),
