...
export async function GET(
  ...
  const rateLimitResult = await rateLimit(`download_v1:${ip}`, {
    limit: 100,
    window: 3600,
    failOpen: false,
  });
  ...
  try {
    ...
    if (type === "media") {
      ...
      if (item.event.visibility !== "public") {
        return new Response("Not Found", { status: 404 });
      }
      ...
      if (!event || event.visibility !== "public" || !event.bannerS3Key) {
        return new Response("Not Found", { status: 404 });
      }
      ...
      if (
        !seriesItem ||
        seriesItem.visibility !== "public" ||
        !seriesItem.bannerS3Key
      ) {
        return new Response("Not Found", { status: 404 });
      }
